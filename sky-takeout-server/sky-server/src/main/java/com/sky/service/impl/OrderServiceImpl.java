...
//    }
    ...
    public void repetition(Long id) {
        ...
        for (OrderDetail orderDetail : orderDetailList) {
            ...
            BeanUtils.copyProperties(orderDetail, newShoppingCart);
            Long userId = BaseContext.getCurrentId();
            newShoppingCart.setUserId(userId);
            List<ShoppingCart> shoppingCartList = shoppingCartMapper.getShoppingCart(newShoppingCart);
            if (shoppingCartList != null && shoppingCartList.size() > 0) {
                ShoppingCart oldShoppingCart = shoppingCartList.get(0);
                newShoppingCart.setNumber(oldShoppingCart.getNumber() + newShoppingCart.getNumber());
                shoppingCartMapper.updateShoppingCart(newShoppingCart);  // old和new都可以
            } else {  // 点击再来一单时，前端会先清空购物车
